        ...
        if(filter_var($mail, FILTER_VALIDATE_EMAIL)) {
               ...
               if($mailexist == 0) {
                  if($mot_de_passe == $mdp2) {
                    $insertmbr = $bdd->prepare("INSERT INTO membre (nom_utilisateur,pn_utilisateur, mail, mot_de_passe) VALUES(?,?,?,?)");
                    $insertmbr->execute(array($nom_utilisateur, $pn_utilisateur,$mail, $mot_de_passe));
                      $erreur ="Votre compte a bien été créé ! <a href=\"../WorldNews_Projet/seconnecter.php\">Me connecter</a>";
                    }
                    else 
